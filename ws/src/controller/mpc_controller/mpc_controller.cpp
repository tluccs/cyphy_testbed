  ...
  point_of_interest_(Eigen::Matrix<T, 3, 1>::Zero())
{
 ...
 //  &MpcController<T>::pointOfInterestCallback, this);
   //CYPHY:
   setpoint_sub_ = nh_.subscribe(
        setpoint_topic_.c_str(), 1, &MpcController<T>::SetpointCallback, this);
   state_sub_ = nl.subscribe(
        state_topic_.c_str(), 1, &MpcController<T>::StateCallback, this);
...
template <typename T>
void MpcController<T>::SetpointCallback(
  const testbed_msgs::ControlSetpoint::ConstPtr& msg)
{
  point_of_interest_(0) = msg->p.x;
  point_of_interest_(1) = msg->p.y;
  point_of_interest_(2) = msg->p.z;
  mpc_wrapper_.setPointOfInterest(point_of_interest_);
//also have v, a, rpy, brates in msg, unused
}

template <typename T>
void MpcController<T>::StateCallback(
...
*/
//CYPHY : publish roll, pitch, yaw, thrust in control message
   testbed_msgs::ControlStamped control_msg;
  control_msg.header.stamp = ros::Time::now();

  control_msg.control.roll =  input_bounded(INPUT::kRateX);
  control_msg.control.pitch =  input_bounded(INPUT::kRateY);
  control_msg.control.yaw_dot =  input_bounded(INPUT::kRateZ);
  control_msg.control.thrust =  input_bounded(INPUT::kThrust);
